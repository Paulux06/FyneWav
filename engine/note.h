...
public:
    ...
    /**note progress (in samples)*/
    unsigned int m_progress;
    /**note panning (max = 32 767 | min = -32 768)*/
    short m_pan;

    /**Empty constructor (pan=0, length=20, height=0, velocity=52 428)*/
    Note() : m_pan(0), m_length(20), m_height(0), m_velocity(52428), m_progress(0) {}
    /**Height Constructor (pan=0, length=20, velocity=52 428)*/
    Note(unsigned int height) : m_pan(0), m_length(20), m_velocity(52428), m_progress(0)
    {
        m_height = height;
    }
    /**Full constructor*/
    Note(unsigned short height, unsigned short velocity,
         unsigned int length, unsigned short pan) : m_progress(0)
    {
        m_velocity = velocity;
        m_height = height;
        m_length = length;
